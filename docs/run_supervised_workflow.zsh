...
# documented in docs/supervised_workflow.md. This script executes the
# reinforcement-learning data collection, supervised training, evaluation, and
# analysis stages in sequence so a single command reproduces the full
# experiment.
...
# ---------------------------------------------------------------------------
typeset -a RL_LEVELS=(0 1 2 3 4)
typeset -a SUPER_LEVELS=(0 1 2 3 4 all)
typeset -a METHODS=(logistic gradient_boost gradient_boost_enhanced)
RESET_TARGETS=(logs models figs analysis)
MODELS_DIR="models"
LOGS_DIR="logs"
...
# Extra per-stage arguments. Repeat the associated flag to append multiple
# tokens (e.g. --rl-train-extra --device --rl-train-extra cuda).
typeset -a RL_TRAIN_EXTRA=()
typeset -a RL_PLAY_EXTRA=()
typeset -a SUPERVISED_EXTRA=()
typeset -a EVAL_EXTRA=()
typeset -a ANALYZE_EXTRA=()

usage() {
  cat <<'USAGE'
Usage: run_supervised_workflow.zsh [options]
...
Reinforcement learning:
  ...
  --rl-train-jobs <n|auto>    Parallel RL training jobs (default: auto).
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --python)
      [[ $# -ge 2 ]] || { echo "run_supervised_workflow.zsh: --python expects a value" >&2; exit 64; }
      PYTHON_BIN=$2
      shift 2
      ;;
    --dry-run)
    ...
    --rl-levels)
      ...
      ;;
    --rl-total-timesteps)
      [[ $# -ge 2 ]] || { echo "run_supervised_workflow.zsh: --rl-total-timesteps expects a value" >&2; exit 64; }
      RL_TOTAL_TIMESTEPS=$2
      shift 2
      ;;
    --rl-seed)
      [[ $# -ge 2 ]] || { echo "run_supervised_workflow.zsh: --rl-seed expects a value" >&2; exit 64; }
      RL_SEED=$2
      shift 2
      ;;
    --rl-wins)
      [[ $# -ge 2 ]] || { echo "run_supervised_workflow.zsh: --rl-wins expects a value" >&2; exit 64; }
      RL_WINS=$2
      shift 2
      ;;
    --rl-deterministic)
    ...
    --no-rl-deterministic)
      ...
      ;;
    --rl-train-extra)
      [[ $# -ge 2 ]] || { echo "run_supervised_workflow.zsh: --rl-train-extra expects a token" >&2; exit 64; }
      append_token RL_TRAIN_EXTRA "$2"
      shift 2
      ;;
    --rl-play-extra)
      [[ $# -ge 2 ]] || { echo "run_supervised_workflow.zsh: --rl-play-extra expects a token" >&2; exit 64; }
      append_token RL_PLAY_EXTRA "$2"
      shift 2
      ;;
    --rl-train-jobs)
      [[ $# -ge 2 ]] || { echo "run_supervised_workflow.zsh: --rl-train-jobs expects a value" >&2; exit 64; }
      RL_TRAIN_JOBS=$2
      shift 2
      ;;
    --rl-play-jobs)
      [[ $# -ge 2 ]] || { echo "run_supervised_workflow.zsh: --rl-play-jobs expects a value" >&2; exit 64; }
      RL_PLAY_JOBS=$2
      shift 2
      ;;
    --super-levels)
      shift
      typeset -a super_values=()
      while [[ $# -gt 0 && $1 != --* ]]; do
        super_values+=("$1")
        shift
      done
    ...
    --methods)
      ...
      ;;
    --models-dir)
      [[ $# -ge 2 ]] || { echo "run_supervised_workflow.zsh: --models-dir expects a value" >&2; exit 64; }
      MODELS_DIR=$2
      shift 2
      ;;
    --max-logs)
      [[ $# -ge 2 ]] || { echo "run_supervised_workflow.zsh: --max-logs expects a value" >&2; exit 64; }
      MAX_LOGS=$2
      shift 2
      ;;
    --wins-only)
    ...
    --no-wins-only)
      ...
      ;;
    --supervised-extra)
      [[ $# -ge 2 ]] || { echo "run_supervised_workflow.zsh: --supervised-extra expects a token" >&2; exit 64; }
      append_token SUPERVISED_EXTRA "$2"
      shift 2
      ;;
    --supervised-jobs)
      [[ $# -ge 2 ]] || { echo "run_supervised_workflow.zsh: --supervised-jobs expects a value" >&2; exit 64; }
      SUPERVISED_JOBS=$2
      shift 2
      ;;
    --episodes)
      [[ $# -ge 2 ]] || { echo "run_supervised_workflow.zsh: --episodes expects a value" >&2; exit 64; }
      EVAL_EPISODES=$2
      shift 2
      ;;
    --max-steps)
      [[ $# -ge 2 ]] || { echo "run_supervised_workflow.zsh: --max-steps expects a value" >&2; exit 64; }
      EVAL_MAX_STEPS=$2
      shift 2
      ;;
    --eval-seed)
      [[ $# -ge 2 ]] || { echo "run_supervised_workflow.zsh: --eval-seed expects a value" >&2; exit 64; }
      EVAL_SEED=$2
      shift 2
      ;;
    --scores-out)
      [[ $# -ge 2 ]] || { echo "run_supervised_workflow.zsh: --scores-out expects a value" >&2; exit 64; }
      SCORES_OUT=$2
      shift 2
      ;;
    --eval-extra)
      [[ $# -ge 2 ]] || { echo "run_supervised_workflow.zsh: --eval-extra expects a token" >&2; exit 64; }
      append_token EVAL_EXTRA "$2"
      shift 2
      ;;
    --eval-jobs)
      [[ $# -ge 2 ]] || { echo "run_supervised_workflow.zsh: --eval-jobs expects a value" >&2; exit 64; }
      EVAL_JOBS=$2
      shift 2
      ;;
    --analysis-dir)
      [[ $# -ge 2 ]] || { echo "run_supervised_workflow.zsh: --analysis-dir expects a value" >&2; exit 64; }
      ANALYSIS_DIR=$2
      shift 2
      ;;
    --analyze-extra)
      [[ $# -ge 2 ]] || { echo "run_supervised_workflow.zsh: --analyze-extra expects a token" >&2; exit 64; }
    ...
    --)
      ...
      ;;
    *)
      echo "run_supervised_workflow.zsh: unrecognised argument '$1'" >&2
      exit 64
      ;;
  esac
done
...
if [[ $# -gt 0 ]]; then
  echo "run_supervised_workflow.zsh: unexpected positional arguments: $*" >&2
  exit 64
fi

cd -- "$REPO_ROOT"

typeset CPU_COUNT_CACHE=""

get_cpu_count() {
  if [[ -n $CPU_COUNT_CACHE ]]; then
    print -- -n "$CPU_COUNT_CACHE"
    return
  fi
  local result=""
...
wait_for_slot() {
  local concurrency=$1
  while (( concurrency > 0 && ${#PIDS[@]} >= concurrency )); do
    local pid=${PIDS[1]:-}
    local cmd=${CMDS[1]:-}
    PIDS=(${PIDS[@]:2})
    CMDS=(${CMDS[@]:2})
...
typeset rl_task_count=${#RL_LEVELS[@]}
typeset rl_play_task_count=${#RL_LEVELS[@]}
typeset super_task_count=$(( ${#METHODS[@]} * ${#SUPER_LEVELS[@]} ))

typeset RL_TRAIN_PARALLEL=$(resolve_jobs $RL_TRAIN_JOBS $rl_task_count)
typeset RL_PLAY_PARALLEL=$(resolve_jobs $RL_PLAY_JOBS $rl_play_task_count)
typeset SUPERVISED_PARALLEL=$(resolve_jobs $SUPERVISED_JOBS $super_task_count)
typeset EVAL_PARALLEL=$(resolve_jobs $EVAL_JOBS $(get_cpu_count))
...
if (( ! SKIP_RESET )); then
  ...
  typeset -a reset_cmd=("$PYTHON_BIN" reset_experiment_data.py --yes)
...
if (( ! SKIP_RL_TRAIN )); then
  run_section "Training RL agents"
  if (( RL_TRAIN_PARALLEL <= 1 )); then
    for level in "${RL_LEVELS[@]}"; do
      typeset -a cmd=("$PYTHON_BIN" train_rl_agent.py --level "$level" --total-timesteps "$RL_TOTAL_TIMESTEPS" --seed "$RL_SEED" --save-path "${MODELS_DIR}/dqn_level${level}" --num-envs auto)
      if (( ${#RL_TRAIN_EXTRA[@]} )); then
        cmd+=("${RL_TRAIN_EXTRA[@]}")
      fi
      run_cmd "${cmd[@]}"
    done
...
if (( ! SKIP_RL_PLAY )); then
  run_section "Collecting winning trajectories"
  mkdir -p -- "$LOGS_DIR"
  if (( RL_PLAY_PARALLEL <= 1 )); then
    for level in "${RL_LEVELS[@]}"; do
      typeset -a cmd=("$PYTHON_BIN" rl_play.py --level "$level" --wins "$RL_WINS" --seed "$RL_SEED" --model-path "${MODELS_DIR}/dqn_level${level}.zip")
      if (( RL_DETERMINISTIC )); then
        cmd+=(--deterministic)
      fi
      if (( ${#RL_PLAY_EXTRA[@]} )); then
        cmd+=("${RL_PLAY_EXTRA[@]}")
      fi
      run_cmd "${cmd[@]}"
    done
...
if (( ! SKIP_SUPERVISED )); then
  run_section "Training supervised agents"
  typeset -a cmd=("$PYTHON_BIN" train_supervised_agents.py --output-dir "$MODELS_DIR" --levels "${SUPER_LEVELS[@]}" --methods "${METHODS[@]}")
...
if (( ! SKIP_EVAL )); then
  ...
  typeset -a cmd=("$PYTHON_BIN" evaluate_supervised_agents.py --model-dir "$MODELS_DIR" --episodes "$EVAL_EPISODES" --max-steps "$EVAL_MAX_STEPS" --seed "$EVAL_SEED" --output "$SCORES_OUT" --jobs "$EVAL_PARALLEL")
  if (( ${#EVAL_EXTRA[@]} )); then
    cmd+=("${EVAL_EXTRA[@]}")
  fi
  run_cmd "${cmd[@]}"
...
if (( ! SKIP_ANALYSIS )); then
  ...
  typeset -a cmd=("$PYTHON_BIN" analyze_scores.py --scores "$SCORES_OUT" --output-dir "$ANALYSIS_DIR")
  if (( ${#ANALYZE_EXTRA[@]} )); then
    cmd+=("${ANALYZE_EXTRA[@]}")
  fi
  run_cmd "${cmd[@]}"
fi
...
if (( DRY_RUN )); then
  print -- "[workflow] Dry-run complete. No commands were executed."
else
  print -- "[workflow] Experiment pipeline finished. Artefacts live in:"
  print -- "  Models:    $MODELS_DIR"
  print -- "  Logs:      $LOGS_DIR"
  print -- "  Scores:    $SCORES_OUT"
